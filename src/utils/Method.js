...
const RedisHandler=require('./RedisUtil');
const fs=require('fs');
...
module.exports={
    ...
    async get(database,table,where,paras){
        ...
        let _path = path.join(__dirname, '../model/' + database + '/' + table + '.js');
        ...
        await RedisHandler.instance.exec('hset', key, paras, JSON.stringify(res));
        return res;
    },
    ...
    async update(database,table,where,paras){
        ...
        res.forEach(v=>{
            promiseList.push((
                async (v)=>{
                    await RedisHandler.instance.exec('hset',key,v.id,JSON.stringify(v));
                }
            )(v));
        })
